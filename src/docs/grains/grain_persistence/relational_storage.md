...
Setting up the connection strings is done as explained in [Orleans Configuration Guide](~/docs/host/configuration_guide/index.md).
...
Read the [ADO.NET Configuration](~/docs/host/configuration_guide/adonet_configuration.md) article for information on configuring your database, including the corresponding ADO.NET Invariant and the setup scripts.
...
`Invariant` (see [ADO.NET Configuration](~/docs/host/configuration_guide/adonet_configuration.md)) that identifies the vendor. You may also choose the format in which the data
...
{
    /// <summary>
    /// Define the property of the connection string for AdoNet storage.
    /// </summary>
    [Redact]
...
These principles are also [included in the database scripts](~/docs/host/configuration_guide/adonet_configuration.md).
